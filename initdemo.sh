...
# Download 0.14.4 specifically, which is referenced by some of the python scripts
docker compose -f docker/compose.yml exec octopus sh -c 'mkdir -p "/terraformcache/registry.terraform.io/octopusdeploylabs/octopusdeploy/0.14.4/linux_amd64"; cd "/terraformcache/registry.terraform.io/octopusdeploylabs/octopusdeploy/0.14.4/linux_amd64"; if [ ! -f "terraform-provider-octopusdeploy_v0.14.4" ]; then curl --silent -L -o "terraform-provider-octopusdeploy_0.14.4_linux_amd64.zip" "https://github.com/OctopusDeployLabs/terraform-provider-octopusdeploy/releases/download/v0.14.4/terraform-provider-octopusdeploy_0.14.4_linux_amd64.zip"; unzip -o "terraform-provider-octopusdeploy_0.14.4_linux_amd64.zip"; rm "terraform-provider-octopusdeploy_0.14.4_linux_amd64.zip"; fi'
