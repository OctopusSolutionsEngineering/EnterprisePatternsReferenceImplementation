...
docker run -e "PGPASSWORD=terraform" --entrypoint '/usr/bin/flock' postgres /root/createdb.lock /usr/bin/psql -h $${DATABASE} -v ON_ERROR_STOP=1 --username "terraform" -c "CREATE DATABASE project_initialize_k8s_space" 2>&1
exit 0
EOT
      }
      environments          = []
