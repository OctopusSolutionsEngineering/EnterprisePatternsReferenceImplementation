...
docker run -e "PGPASSWORD=terraform" --entrypoint '/usr/bin/flock' postgres /root/createdb.lock /bin/bash -c "echo \"SELECT 'CREATE DATABASE project_initialize_k8s_space' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'project_initialize_k8s_space')\gexec\" | /usr/bin/psql -h $${DATABASE} -v ON_ERROR_STOP=1 --username 'terraform'"
exit 0
EOT
      }
      environments          = []
