...
docker run -e "PGPASSWORD=terraform" --entrypoint '/usr/bin/psql' '/usr/bin/flock' postgres /root/createdb.lock /bin/bash -c "/usr/bin/psql $${DATABASE} -v ON_ERROR_STOP=1 --username 'terraform' -c 'CREATE DATABASE ${local.backend}'; sleep 3" 2>&1
exit 0
EOT
      }
      environments          = []
